...
RUN CGO_ENABLED=0 GOOS=linux go build \
    ...
    -o /app/bin/sentiric-identity-service ./cmd/identity-service
...
COPY --from=builder /app/bin/sentiric-identity-service .
RUN chown appuser:appgroup ./sentiric-identity-service

# GÜVENLİK: Kullanıcıyı değiştir
USER appuser

ENTRYPOINT ["./sentiric-identity-service"]