// sentiric-identity-service/cmd/identity-service/main.go
package main
...
import (
	...
	"github.com/sentiric/sentiric-identity-service/internal/service" // Yeni Handler Import
...
func main() {
	...
	log.Info().
		...
		Str("profile", cfg.Env).
		Msg("🚀 Sentiric Identity Service başlatılıyor...")

	// Service Handler'ı oluştur (JWT Manager'ı dahil)
	identityHandler := service.NewIdentityHandler(cfg, log)
	...
	userv1.RegisterIdentityServiceServer(grpcServer, identityHandler)
